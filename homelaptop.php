...
$ram=(int)$ram;
$HDD=$_POST['HDD'];
//$imageData = addslashes(file_get_contents($_FILES["image_1"]["tmp_name"]));
// $gpu=$_POST['gpu'];
// $gpu=(int)$gpu;
...
print_r($HDD);*/
 ...
 //echo $ram;
  ...
  $hdd_price=(int)(6*$budget/10);

  $qry1="SELECT * FROM lram WHERE rsize=$ram and rprice<=$ram_price";
...
//$cost=$cost+$rsolutions[$numrows-1]['rprice'];
  ...
  $qry1="SELECT * FROM lhdd WHERE hsize='$HDD' and hprice<=$hdd_price"; 
  $executequery=mysqli_query($connect,$qry1) or die(mysqli_errno($connect));
  $numrows=mysqli_num_rows($executequery);
  $random= rand(0,$numrows-1);
  $hdd_no=$random;
...
for($i=0;$i<2;$i++) {
 ...
 // $gpu_no1=$gpu_no;
  ...
  <div class="col-sm-5"><img src="limg/'.$id.'.jpeg" style="height:250px; position:relative;left:30px;top:17px;"/></div><div class="col-sm-7" style="color:#7E3992;position:relative;top:20px;"><br>'.$rsolutions[$ram_no]['rname'].'<br>'.$hsolutions[$hdd_no]['hname'].'<br><h3>Total cost:'.$cost.'</h3><br><button id="buy'.$i.'" name="buy" class="btn btn-info btn-lg">Add to Cart</button></div></div><br><br>';
// $mb_no= rand(0,$mb_no);
...
<script type="text/javascript">
  ...
  console.log(ram+hdd+cost+id);
  var urlString="ram="+ram+"&hdd="+hdd+"&cost="+cost+"&id="+id;
  console.log("this is urlString:"+urlString);
  var a=confirm("Add to cart?");
  if(a==1){
$.ajax
  ({
  url: "getlaptopdata.php",
